...
def find_unique_sourceIP_addresses(lines):
    ...
    source_IP_address = set()  # Set to store unique source IPs
    ip_to_list_of_ports = {}  # Dictionary to map IPs to their list of ports

    for words in lines:
        id_resp_h = words[6].strip()
        id_orig_p = words[4].strip()

        # Add the IP to the source_IP_address set
        source_IP_address.add(id_resp_h)

        # Add the port to the corresponding IP in ip_to_list_of_ports if it is not in id_resp_h
        if id_resp_h not in ip_to_list_of_ports:
            ip_to_list_of_ports[id_resp_h] = set()  # Initialize a new set for ports
        ip_to_list_of_ports[id_resp_h].add(id_orig_p)  # Add the port to the set

    print("Final len", len(source_IP_address))

    # Return the final sets and mappings
    return source_IP_address, ip_to_list_of_ports
...
def  count_unique_ids_compute( lines ):
    ...
    unique_ids = set() # Create a set
    duplicate_count = {} # Create a dictionary

    for line in lines:
        uid = line[2].strip()  # Assuming column index 2 is the unique ID
        if uid in unique_ids: # if the unique ID in lines add it to duplicate_count dictionary
            if uid in duplicate_count:
                duplicate_count[uid] += 1
            else: # Otherwise set the value of the ID to 1
                duplicate_count[uid] = 1
        else: # If the unique ID not in lines, then add it to the unique_ids set
            unique_ids.add(uid)
    # return the length of unique ids and the dictionary duplicate_count
    return len(unique_ids), duplicate_count
...
# c profile the file to see which function slows it down

from cProfile import Profile
from pstats import Stats, SortKey

if __name__ == "__main__":
    # file_path = 'HideSmall.csv.gzProcesses.csv'  # Ensure this path is correct
    lines = read_file_as_list_of_strings_DO_NOT_PROFILE( largeName ) # smallname

    # Profile the main processing function
    with Profile() as profile:
        unquieIDs, duplicates, source_IP_address, ip_to_list_of_ports = main(lines)

    # Print stats directly to the python terminal
    stats = Stats(profile)
    stats.strip_dirs()
    stats.sort_stats(SortKey.TIME)  # Sort by time spent
